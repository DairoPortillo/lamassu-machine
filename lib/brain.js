...
const readFailedFrames = callback => failedScansTmpdirs =>
  Promise.all(_.map(
...
Brain.prototype.scanPDF = function scanPDF () {
  ...
  const saveFailedScans = this.shouldSaveQrAttempts ?
    readFailedFrames(photo => idCardData.addIdDataPhoto(photo)) :
    null
  ...
  switch (tierOrCustom) {
    ...
    case 'external':
      ...
      switch (complianceResult[service]?.answer) {
        ...
        case 'RETRY':
        default:
          this.externalComplianceRetry = true
          return false
      }
    default:
...
Brain.prototype._startAddressScan = function _startAddressScan () {
  ...
  let transitionState = 'scanAddress'

  if (this.tx.cryptoCode === 'BLACKPAY') transitionState = 'scanAddressWallet'
  
  this._transitionState(transitionState)
  const txId = this.tx.id

  this.state === 'scanAddress'

  const frames = []
  const saveFailedScans = this.shouldSaveQrAttempts ?
    readFailedFrames(photo => frames.push(photo)) :
    null
  this.scanner.scanMainQR(this.tx.cryptoCode, saveFailedScans, (err, address) => {
    ...
    if (err) this.emit('error', err)
    const startState = _.includes(this.state, ['scanAddress', 'scanAddressWallet', 'goodbye'])
    const freshState = this.tx.id === txId && startState
  ...
  this.screenTimeout = setTimeout(() => {
    if (this.state !== 'scanAddress' || this.state !== 'scanAddressWallet') return
    this.scanner.cancel()
  }, QR_TIMEOUT)
}
